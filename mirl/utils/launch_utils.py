...
import mirl
from mirl.utils.logger import * 
from mirl.utils.initialize_utils import get_item, visit_all_items
import mirl.torch_modules.utils as ptu
import warnings
import copy


_mirl_project_dir = osp.join(os.path.dirname(mirl.__file__), os.pardir)
_LOCAL_LOG_DIR = osp.join(_mirl_project_dir, 'data')
...
def _set_config_by_k_v(config, k, v):
    ...
    find = False
    if len(keys) == 2: 
        if keys[0] == "constant": # type of items
