...
# class MixupAug(nn.Module):
#     def __init__(self, n_sample=64, lam_min=0.5, lam_max=0.8, root='/home/qizhou/datasets'):
#         super().__init__()
#         self.n_sample = n_sample
#         self.lam_min = lam_min
#         self.lam_max = lam_max
#         self.data = torchvision.datasets.STL10(root=root).data # 5000, 3, 96, 96
#         self.stl_n, _, self.stl_h, self.stl_w = self.data.shape

#     def forward(self, x):
#         n, c, h, w = x.size()
#         index = np.random.randint(self.stl_n, size=(self.n_sample,))
#         s_1, s_2 = np.random.randint(self.stl_h-h), np.random.randint(self.stl_w-w)
#         e_1, e_2 = s_1+h, s_2+w
#         noise = self.data[index, :, s_1:e_1, s_2:e_2]
#         noise = ptu.from_numpy(noise)
#         index = np.random.randint(self.n_sample, size=(n*c//3))
#         noise = noise[index].reshape(n,c,h,w)
#         lam = ptu.rand(n,1,1,1)*(self.lam_max-self.lam_min) + self.lam_min
#         aug_x = lam * x + (1-lam) * noise
#         return aug_x

class MixupAug(nn.Module):
    def __init__(self, n_sample=64, lam_min=0.2, lam_max=0.6, root='/home/qizhou/datasets'):
        super().__init__()
        self.n_sample = n_sample
        self.lam_min = lam_min
        self.lam_max = lam_max
    ...
    def forward(self, x):
        n, c, h, w = x.size()
        index = np.random.randint(self.stl_n, size=(n,))
        s_1, s_2 = np.random.randint(self.stl_h-h), np.random.randint(self.stl_w-w)
        e_1, e_2 = s_1+h, s_2+w
        noise = self.data[index, :, s_1:e_1, s_2:e_2]
        noise = ptu.from_numpy(noise)
        noise = noise.repeat(1,c//3,1,1)
        lam = ptu.rand(n,1,1,1)*(self.lam_max-self.lam_min) + self.lam_min
        aug_x = lam*x + (1-lam)*noise
        return aug_x
