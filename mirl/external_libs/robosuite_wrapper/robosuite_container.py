...
from robosuite.wrappers.domain_randomization_wrapper import DomainRandomizationWrapper, DEFAULT_CAMERA_ARGS,\
    DEFAULT_COLOR_ARGS, DEFAULT_LIGHTING_ARGS, DEFAULT_DYNAMICS_ARGS
import matplotlib.pyplot as plt
plt.ion()
from mirl.external_libs.robosuite_wrapper.wrappers import TimeLimit, SingleModality, FrameStack
...
class EnvironmentContainerRobosuite(object):
    def __init__(self, config, train=True, seed=None):
        ...
        if self.dr:
            ...
            dr_config['dynamics_randomization_args'] = dict_merge(DEFAULT_DYNAMICS_ARGS,
                                                                  dr_config.get('dynamics_randomization_args', {}))
...
def make_robosuite(robot, randomize_camera, randomize_color, randomize_light, randomize_dynamics, config):
    ...
    config['env']['domain_randomization_config']['randomize_color'] = randomize_color
    config['env']['domain_randomization_config']['randomize_lighting'] = randomize_light
    config['env']['domain_randomization_config']['randomize_dynamics'] = randomize_dynamics

    env = EnvironmentContainerRobosuite(config['env'], seed=config['seed'])
    env = SingleModality(env, "rgb")
    if config['frame_stack'] and config['frame_stack'] > 0:
...
def make(
    env_name: str,
    robots: Union[str, List[str]] = ['Panda', 'Jaco'],
    mode: str = "train",
    ...
    file_path = dir+'/config/core_imageonly.yaml'
    robfile_path = dir+'/config/robosuite_args.yaml'
    assert mode in ["train", "eval"], print(f"Mode {mode} does not exist")
    with open(file_path) as f:
        config = yaml.safe_load(f)

    config = config['parameters']
    config['seed'] = seed
    config['env']['robosuite_config']['env_name'] = env_name
    config['env']['robosuite_config']['horizon'] = episode_length
    config['env']['robosuite_config']['camera_heights'] = image_height
    config['env']['robosuite_config']['camera_widths'] = image_width
    config['env']['image_key'] = config['env']['robosuite_config']['camera_names'] + config['env']['image_key']

    if mode == "eval":
        with open(robfile_path) as f:
            eval_cfg = yaml.safe_load(f)
        config['env']['domain_randomization_config']['color_randomization_args'] = eval_cfg['color_randomization_args']
        config['env']['domain_randomization_config']['camera_randomization_args'] = eval_cfg['camera_randomization_args']
        config['env']['domain_randomization_config']['lighting_randomization_args'] = eval_cfg['lighting_randomization_args']
        config['env']['domain_randomization_config']['dynamics_randomization_args'] = eval_cfg['dynamics_randomization_args']

    if isinstance(robots, list):
        env = []
        for robot in robots:
            env.append(
                make_robosuite(robot, randomize_camera, randomize_color, randomize_light, randomize_dynamics, config)
            )
    else:
        env = make_robosuite(robots, randomize_camera, randomize_color, randomize_light, randomize_dynamics, config)
    return env
