...
def my_listdir(dir):
  l = os.listdir(dir)
  l = [i.decode('utf-8') for i in l if type(i) is bytes]
  return l
...
def size_and_flatten(image, ref_height, ref_width):
  ...
  if image_height != ref_height or image_width != ref_width:
    ...
    image = cv2.resize(image, (ref_height, ref_width), interpolation=cv2.INTER_LINEAR)
  ...
  def __init__(self,
    ...
    if not dataset_path or num_videos == 0:
      ...
      if not dataset_videos:
        ...
        dataset_videos = sorted(my_listdir(dataset_path))
  ...
  def _reset_background(self):
    ...
    if self._video_paths:
      ...
      if self._shuffle_buffer_size:
        ...
        file_names = my_listdir(video_path)
      ...
      ############ modify ###############
      self.cur_video_path = video_path
