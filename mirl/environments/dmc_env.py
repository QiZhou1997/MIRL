...
import mirl.external_libs.distracting_control.suite_utils as dcs_utils
from mirl.utils.logger import logger
from mirl.environments.dmc_wrapper import make
from mirl.utils.launch_utils import recursively_update_config
import os 
import imageio
...
class DMControlEnv(Env):
    ...
    def _get_dcs_kwargs(self, domain_name, dynamic, distracting):
        dcs_kwargs = {}
...
if __name__ == "__main__":
    ...
    record_video(env)
    exit()
    env_kwargs['video_prefix'] = 'camera_video_'
    env_kwargs["distracting"] = "camera"
    env = DMControlEnv(**env_kwargs)
    record_video(env)
    env_kwargs['video_prefix'] = 'color_video_'
    env_kwargs["distracting"] = "color"
    env = DMControlEnv(**env_kwargs)
    record_video(env)
    env_kwargs['video_prefix'] = 'hard_video_'
    env_kwargs["distracting"] = "hard"
    env = DMControlEnv(**env_kwargs)
    record_video(env)
