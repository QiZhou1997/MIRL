...
from mirl.environments.base_env import Env, SimpleEnv

FILE_PREFIX = 'mirl-mujoco-video'

monitor_closer = closer.Closer()

#在最新的gym下不work
class VideoEnv(Env, Wrapper):
    ...
    def recored_video(
        ...
        **agent_step_kwargs
        ...
        step_iter = range(max_path_length)
        if use_tqdm:
            step_iter = tqdm(step_iter)
        for _ in step_iter:
            a, _ = agent.step(o, **agent_step_kwargs)
            o, _, _, d = self.step(a)
            if d:
                break
...
if __name__ == "__main__":
    env = VideoEnv('cheetah','/home/qizhou')
    env.reset()
    env.set_video_name("test")
    for i in range(1000):
