...
from mirl.agents.base_agent import TorchAgent
...
class DDPGAgent(TorchAgent): 
    ...
    def _log_q_info(self, q_target, q_value_ensemble, qf_loss):
        """
        LOG FOR Q FUNCTION
        """
        qf_info = OrderedDict()
        qf_info['qf/loss'] = np.mean(ptu.get_numpy(qf_loss))
    ...
    def compute_policy_loss(self, obs, new_action, action_info, v_pi_kwargs={}):
    ...
    def _log_policy_info(self, new_action, policy_loss, q_pi_mean):
        policy_info = OrderedDict()
        policy_info['policy/loss'] = policy_loss.item()
        policy_info['policy/q_pi'] = q_pi_mean.item()
        if self._log_tb_or_not(): 
    ...
    def plot_value_scatter(self, info=None, prefix=''):
        if info == None:
            info = self.debug_info
        q_target = info['q_target']
        diff = info['diff']
        q_value_ensemble = info['q_value_ensemble']
