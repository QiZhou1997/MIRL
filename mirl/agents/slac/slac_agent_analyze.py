...
class SLACAgent(SACAgent):
    ...
    def compute_q_target(self, actor_next_obs, next_obs, rewards, terminals, v_pi_kwargs={}):
        ...
        return q_target, next_action, alpha * log_prob_next_action
    ...
    def train_from_torch_batch(self, batch, only_update_latent=False, train_latent_ratio=None):
        ...
        with torch.no_grad():
            ...
            z1, z2, _, _ = self.traj_processor.posterior_process(e_2, a_2)
            next_z1, next_z2, _, _ = self.traj_processor.posterior_process(e_2[:,1:], a_2[:,1:])
            next_z1_aug, next_z2_aug, _, _ = self.traj_processor.posterior_process(e_2[:,1:], a_2[:,1:])
            z = torch.cat([z1, z2], -1)
            next_z = torch.cat([next_z1, next_z2], -1)
            next_z_aug = torch.cat([next_z1_aug, next_z2_aug], -1)
            obs_actor = torch.cat( 
        ...
        if self.analyze_embeddings:
            ...
            projection_loss = self._update_projection(z[:,-2,], z[:,-1], s_2[:,-2], s_2[:,-2], "latent")
            self.train_info['latent_to_state/loss'] = projection_loss.item()
            projection_loss = self._update_projection(obs_actor, next_obs_actor, s_2[:,-2], s_2[:,-2], "history")
            self.train_info['history_to_state/loss'] = projection_loss.item()

        #################
        # update critic #
        #################
        q_target, next_action, sub_term = self.compute_q_target(
            next_obs_actor, 
            z[:,-1], 
            r_2[:,-1], 
            t_2[:,-1], 
        ...
        if self._log_tb_or_not():
            with torch.no_grad():
                next_q_target_1 = self.qf_target.value(
                    next_z[:,-1], 
                    next_action, 
                    **self.next_v_pi_kwargs
                )[0] - sub_term
                next_q_target_2 = self.qf_target.value(
                    next_z_aug[:,-1], 
                    next_action, 
                    **self.next_v_pi_kwargs
                )[0] - sub_term
                diff_1 = (q_target - next_q_target_1).detach().cpu().numpy()
                diff_2 = (next_q_target_2 - next_q_target_1).detach().cpu().numpy()

                min_x, max_x = diff_1.min(), diff_1.max()
                fig, ax = plt.subplots()
                ax.grid(ls='--')
                ax.plot([min_x,max_x],[min_x,max_x], color='gray', ls='--', alpha=0.5)
                ax.scatter(diff_1, diff_2, alpha=0.2)
                logger.tb_add_figure("align_bias_vs_stoch_bias", fig, self.num_train_steps)
        #################
