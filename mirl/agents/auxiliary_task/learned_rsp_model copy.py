...
def td_style(reward, next_v, gamma, lam_scale, gamma_transfer, gamma_scale, tmp=1):
    gamma_transfer = torch.softmax(gamma_transfer/tmp, dim=-1)
...
class LearnedRSPModel(nn.Module, Processor):
    ...
    def compute_auxiliary_loss(self, obs, a, r ,next_obs, 
        ...
        mask = ptu.zeros(self.seq_len)
        mask[0] = 1
        target_q = target_q*(1-mask)+mask
        # end
        ...
        if self.cl_coef > 0:
            target_q, target_r, target_next = td_style(r, next_target_q, self.gamma, 
                self.lam_scale, self.gamma_transfer, self.gamma_scale, self.tmp)
